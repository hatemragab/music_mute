...
from musicmute_worker.__main__ import run_loop
...
class LoopTests(unittest.TestCase):
    ...
    def test_supported_empty_poll_reconnects_without_fifteen_second_sleep(self):
        ...
        worker = Mock(
            config=SimpleNamespace(claim_wait_seconds=25), long_poll_supported=True
        )
    ...
    def test_legacy_api_is_polled_at_one_second(self):
        stop = Mock()
        stop.is_set.side_effect = [False, True]
        worker = Mock(
            config=SimpleNamespace(claim_wait_seconds=25), long_poll_supported=False
        )
        worker.run_once.return_value = "idle"
