...
SEPARATOR = ROOT / "backend" / "separate.py"
if not SEPARATOR.exists():
    SEPARATOR = Path(__file__).resolve().parents[1] / "separate.py"
...
class SeparatorIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        package = self.directory / "audio_separator"
        package.mkdir()
        (package / "__init__.py").write_text("")
        self.loads = self.directory / "loads.txt"
        self.commands = self.directory / "commands.jsonl"
        (package / "separator.py").write_text(f"""
from pathlib import Path
import soundfile as sf
class Separator:
    def __init__(self, **settings):
        assert settings == {{'use_directml':True,'output_dir':settings['output_dir'],'output_format':'FLAC','output_single_stem':'Vocals'}}
    ...
    def load_model(self, model_filename):
        assert model_filename=='Kim_Vocal_2.onnx'
...
""")
    ...
    def test_real_server_loads_model_once_for_two_jobs_and_encodes_only_once_each(self):
        ...
        engine = SeparatorEngine(self.wrapper, self.directory / "state")
