...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path.endswith("truncated"):
            ...
            self.end_headers()
            self.wfile.write(
                b'{"workerId":"z440","state":"enabled","protocolVersion":2}'
            )
...
class TransportTests(unittest.TestCase):
    ...
    def test_api_empty_queue_and_safe_error(self):
        ...
        self.assertEqual(
            Handler.requests[0][1]["User-Agent"], "MusicMuteWindowsWorker/1.0"
        )
    ...
    def test_identity_request_sends_an_authenticated_empty_body(self):
        ...
        self.assertEqual(
            api.post("identity", {}),
            {"workerId": "z440", "state": "enabled", "protocolVersion": 2},
        )
    ...
    def test_configuration_requires_https_and_external_secret(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            path.write_text(
                json.dumps({"api_base_url": "https://api.example.com/api/v1"})
            )
            config = Config.load(path)
            self.assertEqual(config.api_base_url, "https://api.example.com/api/v1")
            self.assertEqual(getattr(config, "worker_id", None), "z440")
            path.write_text(
                json.dumps(
                    {
            ...
            try:
                ...
                self.fail(f"explicit worker_id was rejected: {error}")
            self.assertEqual(config.worker_id, "gpu-02")
            for invalid in ("", "GPU-02", "-gpu", "gpu_02", "a" * 65):
                ...
                with self.subTest(invalid=invalid), self.assertRaisesRegex(
                    ValueError, "worker_id"
                ):
                    Config.load(path)
            path.write_text(json.dumps({"api_base_url": "http://example.com"}))
            with self.assertRaises(ValueError):
                Config.load(path)
            path.write_text(
                json.dumps(
                    {"api_base_url": "https://api.example.com/api/v1", "secret": "bad"}
                )
            )
            with self.assertRaises(ValueError):
                Config.load(path)
