                ...
                for (identifier, prepared, duration), repeat in items:
                    ...
                    if engine is None:
                        engine = engine_factory(
                            config.separator, destination / "ipc", slot=slot
                        )
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Offline Windows separator benchmark")
    ...
    try:
        ...
        with SingleInstance(config.state_dir), KeepAwake(), ProcessRunner() as runner:
            runner.recover()
