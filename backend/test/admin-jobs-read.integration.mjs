...
test(
  'administrative job pages preserve tie ordering and privacy',
  { timeout: 30000 },
  async () => {
    ...
    try {
      ...
      await Promise.all([jobs, users].map((m) => m.init()));
      ...
      const service = new AdminJobsQueryService(jobs, users),
        actor = {
          uid: 'fixture-admin',
          role: 'support',
      ...
      assert.equal(JSON.stringify(first).includes('private'), false);
      await jobs.create(make(6, { createdAt: new Date() }));
