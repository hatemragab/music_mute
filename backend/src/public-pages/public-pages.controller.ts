...
<p>Open Account settings, choose Delete account, authenticate again with a linked sign-in method, then confirm. Account access ends immediately and permanent deletion is automatically scheduled for three calendar months later. Email verification for processing is not required to request deletion. A disabled account can use the support route above.</p></section>
<section aria-labelledby="scope"><h2 id="scope">What deletion covers</h2>
<p>When the three-month recovery period ends, deletion removes the account/profile, associated cloud input audio and vocals-only results, processing history, and account-linked device and push records. The deletion process stops new account work before storage cleanup.</p>
...
<p>Confirmed inputs upload to private Amazon S3 storage. A separately operated Z440 processing machine downloads the input, creates vocals-only audio, and uploads the result. Processing is remote; this is not an entirely offline service. Results, source metadata, filenames, duration, size, and processing status support your account history and downloads.</p>
...
<p>Operational logs, bounded error reports and request/security metadata support service reliability, abuse prevention, and recovery. Infrastructure services may process network and request metadata. Firebase/Google supports identity and messaging; MongoDB holds account/job records; S3 holds private media; the Z440 worker temporarily holds processing inputs and outputs.</p>
