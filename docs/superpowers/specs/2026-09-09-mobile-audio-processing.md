...
The implemented [API](../../../backend/docs/api/audio-processing.md) and [operations requirements](../../../backend/docs/operations/audio-processing.md) are authoritative. Mobile clients never possess AWS credentials or download YouTube URLs on the server.
...
Use isolated emulator-auth/backend/storage fixtures for functional integration; do not create real accounts without approval. Simulator notification injection proves client handling, not live FCM/APNs. Real S3 and a deployed backend need their own end-to-end proof.
