...
| C7  | `PASS` | Backend, integrations, dashboard, Android, compile-only iOS, JSON, relative-link, obsolete-symbol, and whitespace gates passed                | None           |
...
| repository root     | JSON parse, relative Markdown-link audit, obsolete-symbol search, and `git diff --check` |    0 | Documentation and whitespace gates passed                                                  |
...
- Runtime simulator/device and UI E2E remain deferred by maintainer instruction;
  Android JVM tests and iOS app/test-target compilation passed.
- Live provider/deployment checks remain outside this branch and are `NOT_RUN`.
