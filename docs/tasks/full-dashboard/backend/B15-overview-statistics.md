...
**Goal:** Provide accurate retained processing metrics from persisted facts, with clear time semantics and no invented analytics.\
...
- [ ] 3. Aggregate retained job status at `asOf` and historical counts over the requested interval. Preserve unknown duration as null. Optional release summary is omitted if the actor lacks `releases.read`.
...
- [ ] 6. Test each role's projection, invalid dates, empty datasets and legacy job history. Publish metric definitions alongside API docs.
...
Scenario 3: A release manager can view aggregate workload without receiving user identities.
```
