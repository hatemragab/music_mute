...
class Journal:
    def __init__(self, root: Path):
        ...
        if self.path.exists():
            ...
            if (
                ...
                or set(data) != {"schemaVersion", "assignment"}
                or type(data["schemaVersion"]) is not int
                or data["schemaVersion"] != 3
            ):
                raise RuntimeError(
                    "Incompatible assignment schema; re-enroll without erasing journals"
                )
            self.active = (
                selectors(data["assignment"])
    ...
    def save(self, assignment: dict | None) -> None:
        ...
        atomic_json(self.path, {"schemaVersion": 3, "assignment": value})
        self.active = value
...
def inspect_audio(
    ...
    *,
    ...
    try:
        ...
        if any(
            not tool.is_absolute() or tool.is_symlink() or not tool.is_file()
...
class Worker:
    def _media_tools(self):
        ...
        if self.runtime is None or self.runtime.ffprobe is None:
            raise ProfileError("DEPENDENCY_RECIPE_UNAVAILABLE")
        return {"ffmpeg": self.runtime.ffmpeg, "ffprobe": self.runtime.ffprobe}
    ...
    def __init__(
        ...
        *,
        ...
        self.stop = stop or threading.Event()
        bind_installation(config)
        self.journal = Journal(config.state_dir)
        self.progress = Progress(config.state_dir, config.separator, runtime)
        ...
        self.update_hold = ClaimHold(
            DurableRecords(config.paths.state / "updates", create=False)
        )
    ...
    def _validate_claim_runtime(self):
        ...
        if self.runtime is None:
            raise ProfileError("DEPENDENCY_RECIPE_UNAVAILABLE")
        self.runtime.assert_claim_ready()

    def _claim(self, wait_seconds: int = 0):
        # Invalidate the last empty recovery observation before any new request;
        # a concurrent hold cannot mistake an in-flight fresh claim for idle.
        self._update_ownership_resolved = False
        body = {"sessionId": self.session}
        if self._media_policy_supported:
            body["mediaPolicyVersion"] = 2
        from .profiles import ProfileError

        hold = getattr(self, "update_hold", None)
        if hold is not None and hold.held:
            self._update_ownership_resolved = False
            reply = self.api.post("claim/recovery", body)
            self._update_ownership_resolved = reply is None
            return reply
        try:
            self._validate_claim_runtime()
        except ProfileError:
            # This authenticated endpoint can only discover existing ownership.
            # A lost claim reply must not enable fresh admission while unqualified.
            reply = self.api.post("claim/recovery", body)
            if not reply:
                raise
            return reply
    ...
    def update_boundary(self):
        """Only after an authenticated recovery-only discovery in this lifecycle.

        Native launcher additionally verifies child containment before activation;
        this reports the worker's journal/cleanup half of that boundary.
        """
        ...
        if self.cleanup_path.exists():
            ...
            cleanup = value == {"schemaVersion": 3, "cleanup": None}
        else:
            cleanup = self.journal.active is None
        execution = self.execution.data
        acknowledged = execution is None or execution.get("acknowledged") is True
        return SafeBoundary(
            ownership_resolved=getattr(self, "_update_ownership_resolved", False),
            terminal=self.journal.active is None,
            cleanup_acknowledged=cleanup and acknowledged,
            descendants_stopped=False,  # Only native launcher can supply this proof.
        )

    def reconcile_for_maintenance(self, *, stopped_confirmed: bool):
        """Stopped-host recovery only: reuse journals/receipts, never run inference.

        Shared setup invokes this only after native containment verification and
        with the persistent claim hold installed. Live children report their own
        update_boundary over authenticated native IPC instead of sharing journals.
        """
        if stopped_confirmed is not True or not self.update_hold.held:
            raise RuntimeError(
                "Maintenance recovery requires stopped containment and hold"
            )
        self._close_engine()
        self.execution.recover_completed_phase(stopped_confirmed=True)
        self._verify_identity()
        for _ in range(3):
            reply = self._assignment(0)
            boundary = self.update_boundary()
            if (
                boundary.ownership_resolved
                and boundary.terminal
                and boundary.cleanup_acknowledged
            ):
                return boundary
            if reply and reply.get("status") not in TERMINAL:
                break  # Retain existing ownership; never execute it in this host.
        return self.update_boundary()
    ...
    def _verify_identity(self) -> None:
        ...
        if (
            ...
            or reply.get("protocolVersion") != 3
            or reply.get("installationId") != self.config.installation_id
    ...
    def _finish_cleanup(self, assignment: dict, status: str):
        ...
        atomic_json(self.cleanup_path, {"schemaVersion": 3, "cleanup": value})
        self._resume_cleanup()

    def _capture_processing_activation(self, assignment):
        """Bind evidence before work, preserving that binding across attempt recovery."""
        from .update.activation import ActivePointer
        from .update.journal import DurableRecords

        journals = DurableRecords(self.config.state_dir, create=False)
        previous = journals.read("processing-activation.json")
        if previous and previous.get("attemptId") == assignment["attemptId"]:
            return
        records = DurableRecords(self.config.paths.state / "updates", create=False)
        activation = records.read("activation.json")
        if (
            not activation
            or activation.get("phase") != "committed"
            or activation.get("stage") not in ("running", "verified")
        ):
            return
        active = ActivePointer(records).read()
        if activation["candidate"] != active:
            return
        atomic_json(
            journals.root / "processing-activation.json",
            {
                "schemaVersion": 3,
                "releaseId": active["target"]["releaseId"],
                "policyRevision": activation["policy"]["policyRevision"],
    ...
    def _resume_cleanup(self):
        ...
        if (
            ...
            or set(value) != {"schemaVersion", "cleanup"}
            or type(value["schemaVersion"]) is not int
            or value["schemaVersion"] != 3
        ):
            raise RuntimeError(
                "Incompatible cleanup schema; re-enroll without erasing journals"
            )
        value = value["cleanup"]
        if value is None:
            return None
        ...
        atomic_json(self.cleanup_path, {"schemaVersion": 3, "cleanup": None})
        if value["status"] == "ready":
            from .update.journal import DurableRecords

            records = DurableRecords(self.config.paths.state / "updates", create=False)
            captured = DurableRecords(self.config.state_dir, create=False).read(
                "processing-activation.json"
            )
            if captured and captured.get("attemptId") == assignment["attemptId"]:
                atomic_json(
                    records.root / "completed-processing.json",
                    captured,
                )
    ...
    def run_once(self, *, wait_seconds: int = 0) -> str:
        with ProcessRunner() as runner:
            ...
            # Recovery may locate an already-owned assignment under an unavailable
            # runtime. Preserve its journal, but never download/infer new media.
            self._validate_claim_runtime()
            self._capture_processing_activation(assignment)
